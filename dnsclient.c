...
char *query(char *host, size_t domain_length, char *name, char *query_type){

	char *q;
	char *query_str ="POST /dns-query HTTP/1.1\r\nHost: %s\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: %lu\r\nIam: oate1\r\n\r\nName=%s&Type=%s";
	/*determining content length*/
	char *content = "Name=&Type=";
	domain_length = domain_length + strlen(content) + strlen(query_type);
	q = malloc(snprintf(NULL, 0, query_str, host, domain_length, name, query_type) + 1);
	snprintf(q, snprintf(NULL, 0, query_str, host, domain_length, name, query_type) + 1, query_str, host, domain_length, name);
/*	printf("the query: %s\n", q);*/
	return q;
}
...
int main(int argc, char **argv){
	...
	if (argc != 5){
		fprintf(stderr, "usage: a.out IP_address/hostname port www.domain.com A|AAAA|MX\n");
		return 1;
	}
	...
	q = query(argv[1], size_domain, argv[3], argv[4]);
